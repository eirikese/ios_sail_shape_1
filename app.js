...
document.getElementById('startButton').addEventListener('click', () => {
  ...
  // Ensure OpenCV.js has loaded
  if (typeof cv === 'undefined') {
    alert('Error: OpenCV.js is not loaded properly.');
    return;
  }
  ...
  function processVideo() {
    try {
      ...
      if (ids.size().height > 0) {
        ...
        // Reset angles if no markers are detected
        smoothedAngles.r0y = applySmoothing(0, smoothedAngles.r0y);
        smoothedAngles.r1y = applySmoothing(0, smoothedAngles.r1y);
        smoothedAngles.r2x = applySmoothing(0, smoothedAngles.r2x);
        smoothedAngles.r2y = applySmoothing(0, smoothedAngles.r2y);
      }
...
function updateReferenceMarker() {
  ...
  let refInv = new THREE.Matrix4().copy(refMarker.rotation).invert();
...
function updateScene() {
  ...
  for (let i = 0; i < 3; i++) {
    let point = i === 0 ? P0.clone() : i === 1 ? P1.clone() : P2_initial.clone();
    let axis = axes[i];
    let mesh = points[i];
